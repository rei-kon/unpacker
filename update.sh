...
# запомнить текущий релиз для откатa → checkout → uv sync → рестарт ботов.
...
# Запоминаем, откуда ушли — это и есть точка откатa. Пишем ДО checkout: если переключение
# сорвётся на середине, откат всё равно будет знать адрес возврата.
if [ "$ROLLBACK" != "true" ]; then
  if [ "$DRY_RUN" = "true" ]; then
    echo "[dry-run] запомнил бы точку откатa ($CURRENT) в $PREV_FILE"
  else
    printf '%s\n' "$CURRENT" > "$PREV_FILE"
  fi
fi
