...
class TelegramBot:
    ...
    async def _on_speech(self, message: Message) -> None:
        ...
        не только для голосовых. В лимит Bot API (20 МБ на скачивание) упирается и голос:
        замер на живой записи дал ~1,2 МБ на минуту, то есть предел наступает где-то на
        пятнадцатой минуте. Битрейт зависит от клиента, так что цифра ориентировочная —
        но «голосовые в лимит не упираются» неверно, а музыкальный файл или WAV с диктофона
        перебирает 20 МБ и подавно.
