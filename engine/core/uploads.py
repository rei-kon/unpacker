...
def frame_forwarded_text(*, text: str, origin: str | None) -> str:
    """Собрать промпт из пересланного ТЕКСТА с untrusted-рамкой и источником (§8.2).

    Дыра, которую это закрывает: рамку получали файлы и пересланные голосовые, а обычный
    пересланный текст уходил агенту голым — то есть неотличимо от слов самого владельца.
    Владелец пересылает боту чужие идеи из чатов, и «сделай…» внутри такой цитаты читалось
    как его собственная просьба.

    Источник называем отдельной строкой, а не приклеиваем к тексту: агенту важно не только
