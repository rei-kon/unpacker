...
class Settings(BaseSettings):
    ...
    # Распознавание речи (§5.5): голосовое/аудио/видеозаметка → текст, дальше обычным путём.
    # Та же конвенция, что у остальной косметики: нет ключа — фича не создаётся вовсе, и бот
    # честно отвечает, что такие сообщения не принимает. Так ученик без ключа получает
    # прежнее понятное поведение, а не молчание или загадочную ошибку.
    voice_enabled: bool = True
    deepgram_api_key: str | None = None
    # multi = русский вперемешку с английским. На смешанной речи он точнее чистого `ru`:
    # в режиме одного языка названия сервисов между русскими фразами выпадали.
    voice_language: str = "multi"
    # Словарь подсказок (по строке на слово). Путь относителен инстанс-каталогу, как
    # buttons_path. Нет файла — работаем без словаря.
    voice_keyterms_path: str = "keyterms.txt"
    # Потолок ожидания распознавания. Больше response_timeout ставить незачем: агент всё
    # равно не дождётся.
