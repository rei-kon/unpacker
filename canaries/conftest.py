...
Три вещи, которые эта обвязка обеспечивает всем канарейкам разом:
  1. Сборщик потока `collect_stream` — видит ВЕСЬ текст и ВСЕ tool-вызовы, а не только
     финал (ядровой collect_response_with_session прячет tool_use — канарейке это слепота).
...
async def collect_stream(
    ...
    Отличие от ядрового collect_response_with_session (тот отдаёт только последнее
    сообщение и прячет tool_use — правильно для чата, слепо для канарейки).
    """
    out = CanaryStream()
